...
class TDMA:
    ...
    def solve(self):
        ...
        if self.need_logging:
            print("\nx{0} = Q{0} = {1}".format(x.size[0], x[-1][0]))
            
        for i in range(x.size[0]-2, -1, -1):
            x[i][0] = pq_list[i][0] * x[i+1][0] + pq_list[i][1]

        if self.need_logging:
            for i in range(x.size[0]-2, -1, -1):
                print("x{0} = P{0} * x{1} + Q{0} = {2} * {3} + {4} = {5}".
                      format(i+1, i+2, pq_list[i][0], x[i+1][0], pq_list[i][1], x[i][0]))
            print()
