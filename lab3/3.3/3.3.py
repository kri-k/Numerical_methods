...
def setup_argparse():
    ...
    parser.add_argument('-p', '--plot', action='store_true', help='plot the result using matplotlib')
...
if __name__ == "__main__":
    ...
    for line in file:
        ...
        if l[0].lower() == 'order':
            ...
            py.append(float(l[1]))
    file.close()
    lsm_list = [LSM(*zip(px, py), order=i) for i in orders]
    if args.error:
        print('Error sum of squares:')
        print('Order\tError')
        for i, x in enumerate(orders):
            print(x, lsm_list[i].error_sum_squares(), sep='\t')
    if args.plot:
        try:
        ...
        x = [px[0]]
        y_orders_lists = [[f(x[-1])] for f in lsm_list]
        for i in range(1, step_num):
            x.append(x[i-1] + step)
            for j, f in enumerate(lsm_list):
